...
const BrandIcons: Record<string, React.FC<{ className?: string }>> = {
  roblox: ({ className = "w-6 h-6" }) => (
    <svg viewBox="0 0 24 24" fill="currentColor" className={className}>
      {/* Official Roblox Logo (Rotated square with tilted inner square) */}
...
export const SocialLinks: React.FC<SocialLinksProps> = ({ links, onCopySuccess }) => {
  ...
  const handleCopy = (e: React.MouseEvent, link: SocialLink) => {
    ...
    // Copy the handle or direct url
    const textToCopy = link.handle || link.url;
    navigator.clipboard.writeText(textToCopy).then(() => {
      setCopiedId(link.id);
      onCopySuccess?.(`Copied ${link.name} info: "${textToCopy}"`);
  ...
  return (
    <div className="w-full space-y-3.5">
      <div className="flex items-center justify-between px-1">
        ...
        <span className="text-[11px] text-white/50">Click or Copy</span>
      </div>
      ...
      <div className="grid grid-cols-1 gap-3">
        {links.map((link, index) => {
          ...
          return (
            ...
            >
              {/* iOS Frosted Glass Glow Card for Each Social Link */}
              <a
                href={link.url}
                target="_blank"
                rel="noopener noreferrer"
                className={`relative flex items-center justify-between p-3.5 sm:p-4 rounded-2xl bg-white/[0.08] hover:bg-white/[0.14] dark:bg-slate-950/40 dark:hover:bg-slate-900/60 backdrop-blur-xl border border-white/20 dark:border-white/10 ${styles.borderGlow} ${styles.bgGlow} shadow-[0_4px_20px_rgba(0,0,0,0.15)] transition-all duration-300 transform group-hover:-translate-y-0.5`}
              >
                {/* Specular Edge Glow */}
                <div className="absolute top-0 inset-x-4 h-[1px] bg-gradient-to-r from-transparent via-white/30 to-transparent pointer-events-none" />

                {/* Left Side: Brand Icon & Title + Subtitle */}
                <div className="flex items-center gap-3.5 min-w-0">
                  {/* Brand Icon Box with Glass Ring */}
                  <div className={`flex-shrink-0 w-11 h-11 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center border shadow-inner ${styles.iconBg} group-hover:scale-105 transition-transform duration-300`}>
                    <Icon className="w-6 h-6 sm:w-6.5 sm:h-6.5 drop-shadow-md" />
                  </div>

                  {/* Text Container */}
                  <div className="flex flex-col min-w-0 pr-2">
                    <div className="flex items-center gap-2">
                      <span className={`text-sm sm:text-base font-bold text-white tracking-wide transition-colors ${styles.textAccent}`}>
                        {link.name}
                      </span>
                      {link.badge && (
                        <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border ${styles.badgeBg}`}>
                          {link.badge}
                        </span>
                      )}
                    </div>
                    ...
                    {/* Handle / username tag */}
                    <span className="text-[11px] text-cyan-300/80 font-mono truncate mt-0.5">
                      {link.handle}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-1.5 flex-shrink-0">
                  ...
                  <button
                    ...
                    title={`Copy ${link.name} handle`}
                    aria-label={`Copy ${link.name} username or link`}
                    className="p-2 rounded-xl bg-white/10 hover:bg-white/20 active:scale-95 text-white/80 hover:text-white border border-white/15 transition-all"
                  >
                    {isCopied ? (
                  ...
                  <div className="p-2 rounded-xl bg-cyan-500/20 group-hover:bg-cyan-500/40 text-cyan-200 group-hover:text-white border border-cyan-400/30 transition-all">
